...
mxmlc -debug=true -swf-version=51 -default-background-color=#000000 -default-frame-rate=24 -default-size 1000 550 -output Server.swf -- Server.as
if ($LASTEXITCODE -ne 0) { Pop-Location; throw "COMPILE FAILED" }
Pop-Location

# 2. 打包（只含 Server.swf + 图标）
...
Write-Host "DONE: $ScriptDir\Server.app"
Read-Host
